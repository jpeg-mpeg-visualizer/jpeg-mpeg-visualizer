...
#[allow(dead_code)]
impl<'a> Urls<'a> {
    pub fn base(self) -> Url {self.base_url()}
}
...
fn init_audio(model: &mut Model) {
    ...
    let original_buffer = context.create_buffer(
        1,
        model.length,
        model.bitrate
    ).unwrap();
    ...
    let pcm_original_32f = model.pcm_i16
        .iter()
        .map(|value| {
    ...
    let pcm_ulaw_32f = model.decompressed_pcm_i16_8khz_ulaw
        .iter()
        .map(|value| {
            *value as f32 /  (i16::MAX as f32 + 1.0)
        })
    ...
    let  pcm_alaw_32f = model.decompressed_pcm_i16_8khz_alaw
        .iter()
        .map(|value| {
            *value as f32 /  (i16::MAX as f32 + 1.0)
        })
...
fn draw_frame(model: &Model) {
    ...
    compressed_chart.draw_series(LineSeries::new(
...
fn pause_audio(model: &mut Model) {
    ...
    log(&format!("Stopped at playing at {}. Context time: {}", position, current_time));
...
fn update_compression_button(model: &Model) {
    ...
    match model.compression {
        Compression::ULaw => {
            button.set_inner_text("Switch to ALaw")
        },
        Compression::ALaw => {
            button.set_inner_text("Switch to ULaw")
        }
    }
}
...
fn update_playback_source_button(model: &Model) {
    ...
    match model.playback_source {
        PlaybackSource::Original => {
            button.set_inner_text("Switch playback to G711")
        },
        PlaybackSource::Processed => {
            button.set_inner_text("Switch playback to Original")
        }
    }
}
    ...
    match msg {
        Msg::FileChooserLoadAudio(file) => {
            ...
            orders.perform_cmd(async move {
                let (raw_sound, bitrate, length, sound_8khz, length_8khz) = utils::load_audio(file_blob).await;
        ...
        Msg::SwitchCompression => {
            ...
            if model.player_state.playing() && matches!(model.playback_source, PlaybackSource::Processed) {
                pause_audio(model);
                play_audio(model);
            }
            update_compression_button(model);
