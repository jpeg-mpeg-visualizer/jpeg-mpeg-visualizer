...
pub enum MacroblockInfoKind {
    ...
    Interpolated { direction: (i32, i32), forward: MacroblockContent, backward: MacroblockContent, interpolated: MacroblockContent}
}
...
impl Default for MacroblockContent {
    fn default() -> Self {
        let empty = [0; 64];
        Self { 
            y1: empty, y2: empty, y3: empty, y4: empty, cb: empty, cr: empty}
    }
}
...
pub struct MPEG1 {
    ...
    stats_forward: DecodingStats,
    stats_backward: DecodingStats,
...
impl MPEG1 {
    pub fn from_bytes(bytes: Vec<u8>) -> MPEG1 {
        MPEG1 {
            ...
            stats_backward: Default::default(),
    ...
    fn decode_picture(&mut self) {
        ...
        self.stats_current.picture_type = self.picture_type;
        self.stats_current.block_count = self.block_count;
        self.stats_current.macroblock_count = self.macroblock_count;
        self.stats_current.size = self.pointer - old_pointer;
    ...
    fn decode_macroblock(&mut self) {
        ...
        if macroblock_intra {
            ...
            let is_interpolated = self.picture_type == constants::PICTURE_TYPE_B && self.motion_forward.is_set && self.motion_backward.is_set;
            let kind;
        ...
        if self.picture_type == constants::PICTURE_TYPE_B {
            ...
            if self.motion_forward.is_set {
                ...
                if self.motion_backward.is_set && destination == MacroblockDestination::Current {
                    self.copy_macroblock(backward_h, backward_v, MacroblockDestination::Current, FrameOrder::Backward);
    ...
    fn decode_block(&mut self, block: u16, macroblock_intra: bool) {
        ...
        if macroblock_intra {
            ...
            if n == 1 {
                ...
                self.stats_current.macroblock_info[self.macroblock_address as usize]
                    .encoded_blocks
...
mod test {
    ...
    #[test]
    fn test() {
        let bytes = include_bytes!("../../../../rocky_b_new_small.ts");
